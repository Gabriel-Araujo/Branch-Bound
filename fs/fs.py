...
def __transform_to_model(file) -> Model:
    ...
    model.define_objective_function(list(map(float, lines[1].strip().split(" "))))

    for line in lines[2:]:
        model.add_restriction(list(map(float, line.strip().split(" "))))

    model.set_integer(True)
    return model


def get_model_from(filepath: string) -> Model:
